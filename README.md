...
## API (optional, off critical path)

The browser is authoritative — the demo runs entirely client-side and
does not need this. `src/api/main.py` runs the identical pipeline
server-side (red-flag check first, model not called if one fires) so the
same logic can be curled or scripted (this is what makes
`scripts/run_vignettes.py` possible without a browser).

```bash
PYTHONPATH=. .venv/bin/python -m uvicorn src.api.main:app --port 8000
curl http://localhost:8000/health
curl -X POST http://localhost:8000/predict \
  -H "Content-Type: application/json" \
  -d '{"symptoms": ["itching", "skin_rash"], "age_years": 34, "sex": "M", "lang": "en"}'
```
